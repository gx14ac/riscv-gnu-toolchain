srcdir := /Users/shinta/.ghq/github.com/riscv-collab/riscv-gnu-toolchain
builddir := /Users/shinta/.ghq/github.com/riscv-collab/riscv-gnu-toolchain
INSTALL_DIR := /opt/riscv32
...
ifneq (/opt/homebrew/bin/gsed,/bin/sed)
	PATH := $(builddir)/scripts/wrapper/sed:$(PATH)
endif
ifneq (/opt/homebrew/bin/gawk,/usr/bin/gawk)
...
MULTILIB_GEN := 
ifeq ($(MULTILIB_GEN),)
NEWLIB_MULTILIB_NAMES := rv32imac-ilp32
...
GLIBC_MULTILIB_NAMES := rv32imac-ilp32
GCC_CHECKING_FLAGS := 
...
stamps/build-gcc-linux-stage1: $(GCC_SRCDIR) $(GCC_SRC_GIT) stamps/build-binutils-linux \
                               stamps/build-linux-headers
	if test -f $</contrib/download_prerequisites && test "true" = "true"; then cd $< && ./contrib/download_prerequisites; fi
	rm -rf $@ $(notdir $@)
	mkdir $(notdir $@)
	cd $(notdir $@) && $</configure \
		--target=$(LINUX_TUPLE) \
